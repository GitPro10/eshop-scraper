...
export default function separatePriceAndCurrency(str) {
  const rawArray = str.trim().replace(/\s+/g, '').allReplace(replaceString).match(/[\d\.]+|\D+/g); // separate price and currency as a array from raw string containing both price and currency, ex. "$56.10"
